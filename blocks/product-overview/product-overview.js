...
import { div, h6, p, h3, h5, ul, li, button, span } from '../../scripts/dom-builder.js';
...
// Function to generate star rating SVG based on aggregatedRating
function generateStarRating(aggregatedRating) {
  let starRating = '';
  for (let i = 1; i <= 5; i++) {
      if (i <= aggregatedRating) {
          starRating = span({ class: 'icon icon-star-rating flex items-end' });
      } else {
          starRating = '';
      }
  }
...
export default async function decorate(block) {
  ...
  const hostspecies = rawData.hostspecies;
  ...
  if (main.querySelectorAll('div.section[data-tabname="Overview"]')) {
    const overviewContainer = div(
      ...
      button({ class: 'flex items-end appearance-none my-4' },
      ...
      ul({ class: 'relative flex w-full overflow-x-auto overflow-y-hidden select-none scrollbar-hide flex-nowrap rounded-md gap-4 touch-pan-x' },
        li({ class: 'w-full h-44 bg-white' }, 
          button({ class: 'bg-white w-full border border-interactive-grey-transparent-active rounded-md hover:bg-[#0000000d] cursor-pointer text-left'}, 

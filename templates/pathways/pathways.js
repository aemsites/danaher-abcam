...
const body = div({ class: 'container mx-auto px-8 py-16' }, ul({ class: 'space-y-8' }), paginateIndexes);
...
function decorateLists(lists, currentPage = 1, perPage = perPageList) {
  ...
  ulTag.append(
    ...paginatedList.map((res) => {
      ...
      return (
        li(
          { class: 'tracking-wide' },
          a({ class: 'text-teal-600/90 font-semibold text-lg tracking-wider' }, res.title),
  ...
  if (mainTag.querySelectorAll('li.px-4').length === 0) prevBtn.after(...decoratePaginateIndexes(currentPage));
  else updatedPaginateIndexes(currentPage, mainTag.querySelectorAll('li.px-4'));
  if (currentPage === 1) {
    ...
    prevBtn.addEventListener('click', () => {
      decorateLists(allPathways, (currentPage - 1), perPageList);
    });
  }

...
export default async function decorate(block) {
  [...block.children].forEach((el) => {
    applyClasses(el, 'border-b border-b-[#D8D8D8]');
    const contentDiv = el.querySelector('div');
    applyClasses(contentDiv, 'hidden text-base leading-[23px] tracking-[0.3px]');
    const titleDiv = div(
      { class: 'text-[#378189] font-sans text-2xl font-semibold leading-8 tracking-[-0.5px] flex items-center cursor-pointer gap-6 justify-between py-6 px-0 m-0' },

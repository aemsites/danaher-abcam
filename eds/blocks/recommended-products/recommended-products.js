...
export default async function decorate(block) {
  ...
  if(block.classList.includes('recommended-products')) {
    ...
    if (!allRecommendations) block.closest('.section').remove();
      ...
      try {
        block.append(createSlide(allRecommendations));
      } catch (error) {
    ...
    block.append(createSlide(block.children));
  }
}

import {
	pagerController, searchResultListController, pagetypeFacetController, sortController, sourceFacetController
} from '../../scripts/coveo/controller.js';
import coveoEngines from '../../scripts/coveo/engine.js';
import { a, button, div, img, input, label, p, span, h4, h3 } from '../../scripts/dom-builder.js';
...
// 						p(
// 							{ class: 'm-0' },
// 							// Set the src to the full image URL
...
// 							p({ class: 'space-x-4' },
// 								item.raw.producttags && (Array.isArray(item.raw.producttags) && item.raw.producttags.length > 0 || typeof item.raw.producttags === 'string' && item.raw.producttags.trim().length > 0) ?
// 									span(
// 										{ class: 'text-xs font-semibold text-[#378189] bg-[#EDF6F7] p-2' },
// 										Array.isArray(item.raw.producttags)
// 											? item.raw.producttags.join(', ')
// 											: typeof item.raw.producttags === 'string'
// 												? item.raw.producttags.split(',').join(', ') : ''
// 									) : '',
// 								a({ class: 'text-[10px] font-medium text-black underline underline-offset-1' }, 'What is this?')
// 							),
// 							div({ class: 'flex gap-3.5 items-center' },
// 								p(
// 									{ class: 'text-[#65797C] text-xs font-medium m-0 w-24 flex-shrink-0' }, 'Target',
// 								),
// 								p({ class: 'text-sm font-medium m-0' }, item.raw.target),
// 							),

// 							div({ class: 'flex gap-3.5 items-center mt-1' },
// 								p(
// 									{ class: 'text-[#65797C] text-xs font-medium m-0 w-24 flex-shrink-0' }, 'Applications'
// 								),
// 								p({ class: 'text-sm font-medium m-0' }, (Array.isArray(item.raw.reactivityapplications)
// 									? item.raw.reactivityapplications.join(', ')
// 									: typeof item.raw.reactivityapplications === 'string'
// 										? item.raw.reactivityapplications.split(',').join(', ') : '')),
// 							),
// 							div({ class: 'flex gap-3.5 mt-1' },
// 								p(
// 									{ class: 'text-[#65797C] text-xs font-medium m-0 !w-24 flex-shrink-0' }, 'Reactivity'
// 								),
// 								p({ class: 'text-sm font-medium m-0 w-full' },
// 									(Array.isArray(item.raw.reactivityspecies)
// 										? item.raw.reactivityspecies.join(', ')
// 										: typeof item.raw.reactivityspecies === 'string'
// 											? item.raw.reactivityspecies.split(',').join(', ') : '')
// 								),
// 							),
// 							div({ class: 'flex gap-3.5 items-center mt-1' },
// 								p(
// 									{ class: 'text-[#65797C] text-xs font-medium m-0 w-24 flex-shrink-0' }, 'Host Species'
// 								),
// 								p({ class: 'text-sm font-medium m-0' }, item.raw.hostspecies),
// 							),
// 						),
// 					),
// 					div({ class: 'bg-[#edf6f7] p-6 w-1/4' },
...
// 						),
// 						p({ class: 'mt-4 mb-0' },
// 							a({ class: 'flex gap-2 items-center text-sm text-[#378089] font-bold mt-4', href: '#' }, 'View Product',
					...
					div(
						...
						p({ class: 'flex items-center space-x-4' },
							span({ class: 'flex text-xs items-center' },
								...
								span({ class: 'text-[13px] text-[#378089] underline font-normal ml-2' },
...
function renderSort(element) {
	...
	const dropdownButton = button({ class: 'w-36 h-10 font-semibold text-[#071112] text-sm flex item-center justify-between gap-2 py-2.5 px-6 bg-[#273F3F0D] rounded-3xl outline-none' },
		'Relevance',
	);
	...
	sortOptions.forEach((option) => {
		...
		if (sortController.state.sortCriteria.includes(option.label.toLowerCase())) selectedName = option.label;
